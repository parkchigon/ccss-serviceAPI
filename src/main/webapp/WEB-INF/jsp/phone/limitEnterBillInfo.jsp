...
<script type="text/javascript">
		...
		$.ajax({
			...
			success : function(data) {
				if ("0000" == data.resultCode) {
					showCompleted(data.name, data.pay, data.date);
	...
	function showAlreadyAlert() {
		showAlert("요청하신 요금제로 변경했습니다.<br/>서비스를 이용해 주셔서 감사합니다.");
	}

	function showCompleted(pname, ppay, pdate) {
		$(".dim").show();
		$("#completed").css("display", "block");
		$("#pname").html(pname+$("#pname").html());
		$("#ppay").html(ppay+$("#ppay").html());
		$("#pdate").html(pdate+$("#pdate").html());
		...
		<div class="container">
			<section>
				<div class="data_info_view">
					<dl>
						<dt>아래정보를 모두 입력해 주세요.</dt>
						<dd>
							<div class="d-grid">
								<table>
									<caption>납부방법변경</caption>
									<colgroup>
										<col style="width: 30%">
									...
									<tbody>
										...
										<tr class="input">
											...
											<!-- <td><input type="number" max="999999" maxlength="6"
												...
												<td><input type="text" placeholder="YYYY년MM월" id="yyyymm"
														value="" pattern="\d*" onFocus="this.blur()" /></td>
										</tr>
										<tr>
											<th>납부 금액</th>
											<td class="pay"><span class="txt-red">13,200원</span></td>
										</tr>
									</tbody>
								</table>
							</div>
					...
					<dl>
						<dd>
							<div class="info_view_inner2">
								<ul style="padding-bottom: 76.5px">
									<li>결제일은 매월 12일이고 결제일이 주말 또는 공휴일 인 경우 돌아오는 첫 평일에 결제 됩니다.</li>
									<li>자동결제일 전에 직접 납부하는 경우 중복으로 결제될 수 있으니, 유의해주세요. 중복 결제된 경우
										자동으로 환불되거나 다음달 요금에서 납부한 만큼이 미리 결제됩니다.</li>
									<li>신용카드로 납부하는 경우 휴대폰 또는 아이폰 인증 등의 본인 확인이 필요합니다.</li>
									<li>고객님의 정확한 결제일을 확인하시려면 U+홈페이지 1:1 문의하기로 접수해주세요.</li>
								</ul>
							</div>
						</dd>
					</dl>
			...
			<div id="completed" class="popup2" style="display: none;">
				<div class="popup_inner">
					<p class="title">요금제변경 신청 완료</p>
					<!-- 타이틀 없을경우 삭제 -->
					<div class="info">
						신청하신 요금제로 변경했습니다.<br />서비스를 이용해주셔서 감사합니다.
					</div>
					<div class="p-grid">
						<table>
				...
				<div class="popup_btnArea">
					...
					<!-- 버튼 1개일경우 -->
					<a id="confirms" href="javascript:;" class="btn2">확인</a>
				</div>
			</div>
		</div>
	</div>
