...
@Service("arrivalModiService")
public class ArrivalModiServiceImpl implements ArrivalModiService{

	private static final Logger logger = LoggerFactory.getLogger(ArrivalModiServiceImpl.class);
	private static final List<String> mandatoryList = new ArrayList<String>(
			Arrays.asList(RequestJSON.PARAM_MEMB_ID,RequestJSON.PARAM_USE_YN,RequestJSON.PARAM_ARRIVAL_ALARM_ID));
	...
	public ResponseJSON doService(HttpHeaders headers, RequestJSON reqJson) {
		...
		CheckResultData result = ValidationChecker.checkValidation(headers,reqJson,CCSSConst.ARRIVAL_NOTI_MODIFY,mandatoryList);
		...
		if (result.isStatus() == false) {
			...
			try {
				...
				if(membVO !=null){
					...
					//DB 등록
					boolean regFlag = arrivalNotiDao.updateArrivalAlarmInfo(arrivalAlarmSetVO);
					
					
					if(regFlag){
						logger.info("Arrival Alarm Info Modify Success. sessionId({}) membId({}) mgrappId({})  mgrappLoginId({})",sessionId,membId,mgrappId,mgrappLoginId);
						...
						resultCode=ResultCodeUtil.RC_4C005001;
						logger.error("Arrival Alarm Info Modify Fail.   sessionId({}) membId({}) mgrappId({})  mgrappLoginId({}) arrivalAlarmSetVO({})",sessionId,membId,mgrappId,mgrappLoginId,arrivalAlarmSetVO.toString());
					}
				}else{
