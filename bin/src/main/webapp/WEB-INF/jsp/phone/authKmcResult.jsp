	...
	try {
	 ...
	 String ip = request.getHeader("HTTP_X_FORWARDED_FOR");
		...
		if (ip == null || ip.length() == 0) {
			...
			System.out.println("본인 핸드폰 ip주소.[" + string12 + "]");
	  	
	   	    //사용자IP xxx.xxx.xxx 3번째 자리까지 IP 확인 해서 넘겨 주기
			 boolean IpCheck = 
		            (ip.split("\\.")[0]+ip.split("\\.")[1]+ip.split("\\.")[2]).equals
		            (string12.split("\\.")[0]+string12.split("\\.")[1]+string12.split("\\.")[2]);
		      if (!IpCheck) {
		         TP = "KMC";
		         CI = "";
		         DI = "";
		...
		/* if (!ip.equals(string12)) { // IpCheck 추가하기전 소스 
			TP = "KMC";
			CI = "";
			DI = "";
			PN = "";
